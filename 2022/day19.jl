...
function solve(bp, T, ORE, CLAY, OBSIDIAN, MAXORE, MAXCLAY, MAXOBSIDIAN)
    ...
    dp[1, 0, 0, 0, 0, 0, 0] = 0
    for t in 0:(T - 1)
        for ra in 1:ORE, rb in 0:CLAY, rc in 0:OBSIDIAN, a in 0:MAXORE, b in 0:MAXCLAY,
            c in 0:MAXOBSIDIAN
            ...
            if a >= bp[1][1] && ra < ORE
                idx = CartesianIndex(ra + 1,
                    rb,
                    rc,
                    min(a - bp[1][1] + ra, MAXORE),
                    min(b + rb, MAXCLAY),
                    min(c + rc, MAXOBSIDIAN),
                    t + 1)
                dp[idx] = max(dp[idx], dp[now])
            end

            if a >= bp[2][1] && rb < CLAY
                idx = CartesianIndex(ra,
                    rb + 1,
                    rc,
                    min(a - bp[2][1] + ra, MAXORE),
                    min(b + rb, MAXCLAY),
                    min(c + rc, MAXOBSIDIAN),
                    t + 1)
                dp[idx] = max(dp[idx], dp[now])
            ...
            if a >= bp[3][1] && b >= bp[3][2] && rc < OBSIDIAN
                idx = CartesianIndex(ra,
                    rb,
                    rc + 1,
                    min(a - bp[3][1] + ra, MAXORE),
                    min(b - bp[3][2] + rb, MAXCLAY),
                    min(c + rc, MAXOBSIDIAN),
                    t + 1)
                dp[idx] = max(dp[idx], dp[now])
            end
            ...
            if a >= bp[4][1] && c >= bp[4][2]
                idx = CartesianIndex(ra,
                    rb,
                    rc,
                    min(a - bp[4][1] + ra, MAXORE),
                    min(b + rb, MAXCLAY),
                    min(c - bp[4][2] + rc, MAXOBSIDIAN),
                    t + 1)
